...
- ✅ QA pipeline (2026-06-05): project-scoped **Story → Gherkin → lint → rubric** tool (`/tools/qa-pipeline`, `components/qa/QaPipeline.tsx`, `api/qa-pipeline`, `lib/qaPipeline.ts`). One-shot JSON (no SSE). `loadProjectQaContext(projectId)` resolves the active project's `lbmh-gherkin-authoring` + `lbmh-qa-eval-rubric` templates **by slug** (never hardcodes glossary) and checks for glossary facts; the orchestrator drafts via `chat()` with the project's memory injected (`getMemoryContext`) + the Gherkin template, lints deterministically by **reusing** `lintGherkin()`, and scores via `chat()` + the rubric template. Saves a new `QaRun` model (`projectId`/`input`/`draftFeature`/`lintOk`/`errors`/`warnings`/`score Json`). A project with no pack returns `{ needsPack: true }` (a 200, not a degraded run) → empty-state card pointing to `npm run seed:lbmh`. Verified live on LBMH: a 2-line POS story drafted a 4-scenario tagged `.feature` (one `When` each) using Spruce vocab (POS/tax-exempt/UOM) from the injected glossary, lint flagged one actionable gap, rubric returned a verdict, and the run persisted. lint + build green, 32/32 e2e.
